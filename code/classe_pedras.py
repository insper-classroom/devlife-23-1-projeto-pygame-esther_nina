...
class Pedras():
    def __init__(self):
        ...
        self.pedra_tempo = - 1
        self.rect = pygame.rect.Rect(x, -100, 50, 100)
        self.pedras = []
    ...
    def colisao_bola(self, posicao_bola):
        if (posicao_bola[0] >= self.rect[0] and posicao_bola[0] <= self.rect[0] + 50) and posicao_bola[1] >= self.rect[1] and posicao_bola[1] <= self.rect[1] + 100:
            return True
        else:
            return False
